...
add_action( 'admin_print_styles', 'removeWooNotice' );
/**
 * Remove the default WooCommerce Notice.
 *
 * @since 2.3.0
 */
function removeWooNotice() {
...
add_action( 'admin_notices', 'wooThemeNotice' );
/**
...
add_action( 'wp_ajax_genesis_sample_dismiss_woocommerce_notice', 'DismissWooNotice' );
/**
 * Add option to dismiss Genesis Connect for Woocommerce plugin install prompt.
 *
 * @since 2.3.0
 */
function DismissWooNotice() {
	update_user_option( get_current_user_id(), 'genesis_sample_woocommerce_message_dismissed', 1 );
}

add_action( 'admin_enqueue_scripts', 'wooNoticeScript' );
/**
 * Enqueue script to clear the Genesis Connect for WooCommerce plugin install prompt on dismissal.
 *
 * @since 2.3.0
 */
function wooNoticeScript() {
	wp_enqueue_script( '-woo-notice-script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', array( 'jquery' ), '1.0', true  );
}

add_action( 'switch_theme', 'resetWooNotice', 10, 2 );
/**
 * Clear the Genesis Connect for WooCommerce plugin install prompt on theme change.
 *
 * @since 2.3.0
 */
function resetWooNotice() {
...
add_action( 'deactivated_plugin', 'resetWooNoticeOnDeactivation', 10, 2 );
/**
 ...
 */
function resetWooNoticeOnDeactivation( $plugin, $network_activation ) {
